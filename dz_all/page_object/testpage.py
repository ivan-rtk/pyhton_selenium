...
class OperationsHelper(BasePage):
    ...
    def enter_name(self):
        ...
        name_field = self.find_element(TestSearchLocators.LOCATOR_NAME_FIELD, time=10)
        name_field.clear()
        name_field.send_keys(text)

    def enter_email(self):
        logging.info(f"Send {text} to element {TestSearchLocators.LOCATOR_EMAIL_FIELD[1]}")
        email_field = self.find_element(TestSearchLocators.LOCATOR_EMAIL_FIELD, time=10)
        email_field.clear()
    ...
    def enter_content(self):
        ...
        content_field = self.find_element(TestSearchLocators.LOCATOR_CONTENT_FIELD, time=10)
        content_field.send_keys(text)

    def get_contact_us_btn(self):
        logging.info("Click login button")
